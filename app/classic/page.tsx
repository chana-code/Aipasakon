    ...
    <>
      ...
      <section className="max-w-[1200px] mx-auto px-6 py-20 md:py-32 flex flex-col md:flex-row items-center gap-12">
        <div className="md:w-3/5">
          <h1 className={`text-[40px] font-bold leading-tight text-[#00143C] mb-6`}>
            AI ไม่ยาก<br />ถ้าพูดภาษาคน
          </h1>
          <p className="text-[18px] leading-[1.8] text-[#00143C]/80 mb-10 max-w-[540px]">
          ...
          <div className="flex flex-wrap gap-4">
            <Link href="/what-is-ai" className="bg-[#14B5AB] text-white px-8 py-4 rounded-lg text-[14px] font-medium flex items-center gap-2 hover:shadow-lg hover:-translate-y-0.5 transition-all no-underline">
              เริ่มตรงนี้ <span className="material-symbols-outlined text-sm">arrow_forward</span>
            </Link>
            <Link href="/curriculum" className="border border-[#00143C]/20 text-[#00143C] px-8 py-4 rounded-lg text-[14px] font-medium hover:bg-[#f0eee9] transition-all no-underline">
              ดูหลักสูตรทั้งหมด
            </Link>
          </div>
        </div>
        <div className="md:w-2/5 relative">
          ...
          <div className="absolute -bottom-6 -left-6 bg-white p-6 rounded-lg shadow-xl border border-[#E8E2D4] max-w-[200px]">
            <p className={`text-[18px] text-[#00143C]`}>&ldquo;เนื้อหาที่เปลี่ยนโลกซับซ้อนให้กลายเป็นเรื่องคุยง่าย&rdquo;</p>
          </div>
        </div>
      </section>
      ...
      <section className="bg-[#f5f3ee] py-20 border-y border-[#E8E2D4]">
        <div className="max-w-[1200px] mx-auto px-6">
          <h2 className={`text-[28px] font-semibold leading-[1.3] text-[#00143C] mb-12 text-center`}>เริ่มตรงไหนดี?</h2>
            ...
            {DOORS.map(d => {
              ...
              return (
                <div key={d.level} className="bg-white p-8 rounded-r-lg hover:shadow-md transition-all group" style={{ borderLeft: `3px solid ${hex}` }}>
                  <h3 className={`text-[22px] mb-4 text-[#00143C]`}>{d.title}</h3>
                  <p className="text-[#00143C]/70 mb-8">{d.desc}</p>
                  <Link href={`/${d.level}`} className="text-[14px] font-medium flex items-center gap-2 group-hover:gap-4 transition-all no-underline" style={{ color: hex }}>
      ...
      <section className="max-w-[800px] mx-auto px-6 py-24">
        <div className="text-center mb-16">
          <h2 className={`text-[28px] font-semibold leading-[1.3] text-[#00143C] mb-4`}>เส้นทางการเรียนรู้</h2>
              ...
              <Link key={lvl} href={`/${lvl}`} className="relative z-10 flex gap-8 items-start no-underline group">
                ...
                <div className="flex-1 pt-2">
                  <div className="flex justify-between items-baseline border-b border-[#E8E2D4] pb-2 mb-2">
                    <h4 className={`text-xl group-hover:underline`} style={{ color: hex }}>Level {m.order}: {m.label_th}</h4>
                    <span className="text-xs font-medium uppercase tracking-widest text-[#00143C]/50">{count(lvl)} Chapters</span>
                  </div>
        ...
        <div className="max-w-[1200px] mx-auto px-6 flex flex-col md:flex-row justify-around items-center gap-8 text-center">
          {[['language', 'ภาษาไทยล้วน'], ['verified', 'อ้างอิงได้จริง'], ['business_center', 'เขียนจากประสบการณ์ทำธุรกิจจริง']].map(([icon, label]) => (
            <div key={icon} className="flex items-center gap-3">
              ...
              <span className={`text-lg text-[#00143C]`}>{label}</span>
            </div>
          ))}
        </div>
      </section>
      ...
      {latest.length > 0 && (
        <section className="max-w-[1200px] mx-auto px-6 py-24">
          <div className="flex justify-between items-end mb-12">
            <h2 className={`text-[28px] font-semibold leading-[1.3] text-[#00143C]`}>บทความล่าสุด</h2>
            <Link href="/curriculum" className="text-[#14B5AB] text-[14px] font-medium border-b border-[#14B5AB]/20 hover:border-[#14B5AB] transition-all no-underline">ดูบทความทั้งหมด</Link>
          </div>
            ...
            {latest.map(c => {
              ...
              return (
                <Link key={c.slug} href={`/${c.level}/${c.slug}`} className="bg-white p-6 rounded-lg border border-[#E8E2D4] hover:border-t-[#14B5AB] hover:border-t-4 transition-all duration-300 group no-underline block">
                  ...
                  <h3 className={`text-xl mb-4 text-[#00143C] group-hover:text-[#14B5AB] transition-colors`}>{c.title}</h3>
                  {c.tldr && <p className="text-[#00143C]/70 text-sm mb-6 line-clamp-3">{stripMd(c.tldr)}</p>}
                  <div className="text-xs text-[#00143C]/40">อ่าน {readMin} นาที{c.last_reviewed ? ` • ${c.last_reviewed}` : ''}</div>
                </Link>
              );
            })}
          </div>
